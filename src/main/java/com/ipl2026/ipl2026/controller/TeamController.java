...
public class TeamController {
    ...
    @PatchMapping("/{id}")
    public String updateTeam(@Valid @PathVariable Long id, @RequestBody Team ipl2026){
    ...
    @GetMapping("/nrr/{nrr}")
    public List<Team> filterNrr(@Valid @PathVariable Float nrr){
        return iplService.filterByNrr(nrr);
    }

    @GetMapping("/wins/{wins}")
    public List<Team> filterWins(@Valid @PathVariable Integer wins){
        return iplService.filterByWins(wins);
    }

    @GetMapping("/matches/{matches}")
    public List<Team> filterMatches(@Valid @PathVariable Integer matches){
